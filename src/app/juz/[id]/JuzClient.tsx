...
export default function JuzClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    async function fetchAllData() {
      ...
      try {
        ...
        if (!cancelled) setExtraData(dataMap);
      } catch (e) {
        console.error(e);
      }
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(allAyahsInJuz.length / ITEMS_PER_PAGE),
  );
            ...
            >
              <div className="flex items-center justify-between mb-12 border-b border-slate-50 dark:border-slate-800 pb-8 transition-colors duration-500">
                <div className="flex items-center gap-5">
                  <span
                    className={`w-14 h-14 flex items-center justify-center rounded-2xl font-black text-sm transition-all duration-500 ${
                      isLastRead
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest">
                      {surahNames[ayah.sura]?.mk ?? `Сура ${ayah.sura}`}
                    </p>
                    {isLastRead && (
